...
function getDrawXY(nodeType) {
    ...
    if (nodeType !== 3) {
        ...
        if (parseInt(fenzhiNum) !== 1) {
            ...
            if (topBottom === 'bottom') {
                maxY = 180 + heightBei * 50;
            } else {
                maxY = 180 - heightBei * 50;
            }
            $('#fenzhi_num').val(parseInt(fenzhiNum) + 1);
            return [maxX, maxY]
        } else {
            allNode.forEach(function (node) {
                var x = node.obj.attr("x");
...
function drawMainNode(node) {
    ...
    if(xAndY[0] > defaultPaperWidth){
        defaultPaperWidth += 600;
        paper.setSize(defaultPaperWidth,500);
    }
    //如果是普通节点则不要圆角
...
//     window.localStorage.setItem("nodeInfo",JSON.stringify(nodeInfo));
// }
//
