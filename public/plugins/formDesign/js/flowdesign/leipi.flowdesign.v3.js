...
(function ($) {
    ...
    var initEndPoints = function () {
        $(".process-flag").each(function (i, e) {
            ...
            jsPlumb.makeSource($(e), {
                ...
                paintStyle: defaults.connectorPaintStyle,
        ...
        if (aConnections.length > 0) {
            ...
            for (var j = 0; j < aConnections.length; j++) {
                ...
                if(!isAddIn){
                     ...
                     $("#leipi_process_info").append(tempStr);
                }
            }
    ...
    var setDashConnections = function (conn, remove) {
        if (!remove) dConnections.push(conn);
        else {
            var idx = -1;
            for (var i = 0; i < dConnections.length; i++) {
                if (dConnections[i] == conn) {
                    idx = i;
                    break;
                }
            }
            if (idx != -1) dConnections.splice(idx, 1);
        }
        if (dConnections.length > 0) {
            var s = "";
            for (var j = 0; j < dConnections.length; j++) {
                var from = $('#' + dConnections[j].sourceId).attr('process_id');
                var target = $('#' + dConnections[j].targetId).attr('process_id');
                var insertTo = from + "," + target;
                var isAddIn = $("#leipi_dashProcess_info").find("input[value='"+ insertTo +"']").length;
                if(!isAddIn){
                    var tempStr = "<input type='hidden' value=\"" + from + "," + target + "\">";
                    $('#leipi_dashProcess_info').append(tempStr);
                }
            }
        } else {
            $('#leipi_dashProcess_info').html('');
        }
        // jsPlumb.repaintEverything();//重画
    };
    ...
    $.fn.Flowdesign = function (options) {
        ...
        _canvas.append('<div id="leipi_dashProcess_info"></div>');
        ...
        jsPlumb.draggable(jsPlumb.getSelector("._jsPlumb_connector"));
        initEndPoints();
        ...
        jsPlumb.bind("jsPlumbConnection", function (info) {
            ...
            if(curLine === 'solid'){
                ...
                setDashConnections(info.connection);
            }
        });
        //绑定删除connection事件
        ...
        var _canvas_dash_design = function () {
            var curLine = $("#curLine");
            curLine.val("dash");
            $('.process-step').each(function (i) {
                var sourceId = $(this).attr('process_id');
                //var nodeId = "window"+id;
                var prcsto = $(this).attr('back_to');
                if(prcsto){
                    var toArr = prcsto.split(",");
                    $.each(toArr, function (j, targetId) {
                        if (targetId != '' && targetId != 0) {
                            //检查 source 和 target是否存在
                            var is_source = true, is_target = true;
                            if (is_source && is_target) {
                                ...
                                var newAdd = allPath[allPath.length - 2];
                                if(curLine.val() === 'dash'){
                                    newAdd = $(newAdd).attr("stroke-dasharray","3");
                                }
                                return true;
                            }
                        }
...
//-----外部调用----------------------
        ...
        var Flowdesign = {
            ...
            getProcessInfo: function () {
                ...
                $("#leipi_dashProcess_info input[type=hidden]").each(function (i) {
                    var processVal = $(this).val().split(",");
                    if (processVal.length == 2) {
                        if (!dProcessData[processVal[0]]) {
                            dProcessData[processVal[0]] = {
                                "top": 0,
                                "type": 2,
                                "name": "新建步骤",
                                "left": 0,
                        ...
                        dProcessData[processVal[0]]["back_to"].push(processVal[1]);
                    }
                });
                /*位置*/
                ...
                _canvas.find("div.process-step").each(function (i) { //生成Json字符串，发送到服务器解析
                    if ($(this).attr('id')) {
                        ...
                        if(dProcessData[pId]){
                            resultsProcessData[pId]['back_to'] = dProcessData[pId]['back_to'];
                        }else{
